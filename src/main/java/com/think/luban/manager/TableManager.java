...
public final class TableManager {
    ...
    public static <T, ID extends Serializable, R extends CfgRepository<T, ID>> R getRepository(Class<T> clazz) {
        return (R) tableMap.get(clazz);
    }

    public <T> Optional<T> findById(Class<T> clazz, Predicate<T> predicate) {
        CfgRepository<T, Serializable> repository = getRepository(clazz);
        if (repository == null) {
