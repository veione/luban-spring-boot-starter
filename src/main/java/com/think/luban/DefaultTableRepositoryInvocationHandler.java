...
public class DefaultTableRepositoryInvocationHandler<T> implements CfgRepository<T, Serializable>, InvocationHandler, Reloadable {
    ...
    private final LubanTableProperties tableProperties;
    ...
    public DefaultTableRepositoryInvocationHandler(ApplicationContext applicationContext, Class<T> clazz) {
        ...
        this.tables.register(tableClazz, this);
        this.init();
    }
    ...
    public synchronized void reload() {
        // 先清理之前的资源
        items.clear();
        ITableLoader<JsonElement> loader = tables.getLoader();
        String tableFileName = definition.getTableFileName();
        JsonElement buf = loader.load(tableFileName);

        Map<Serializable, List<T>> indexMap = new HashMap<>();
    ...
    public String toString() {
        return "DefaultTableRepositoryInvocationHandler{" +
                "tables=" + tables +
